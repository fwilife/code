...
int main() {
    ...
    Shader ourShader("texurevs.txt", "texurefs.txt");
    CVAO VA;
    ...
    unsigned char* data = stbi_load("8525003.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("2022.png", &width, &height, &nrChannels, 0);
    if (data)
    {
